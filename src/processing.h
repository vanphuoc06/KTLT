...
double calculate_MaxProfit(Stock &stock, std::string &start_date, std::string &end_date);

//Tìm chiều dài của chuỗi ngày tăng giá liên tiếp dài nhất
int Longest_Increasing_Subarray(Stock &stock, std::string &start_date, std::string &end_date);

//Tìm khoảng thời gian ngắn nhất mà tổng giá trị cổ phiếu >=mục tiêu S(Sliding Window)
int findShortestWindowForTargetProfit(Stock &stock, double &target, std::string &start_date, std::string &end_date);

//Tìm mảng cộng dồn(Prefix Sum) để truy vấn tổng nhanh chóng
std::vector<double> Prefix_Sum(Stock &stock);

//Truy vấn tổng giá trị trong một khoảng [left,right] dùng Prefix Sum
void Query_PrefixSum(std::vector<double> &Prefix_Sum, int left, int right, double &result);
